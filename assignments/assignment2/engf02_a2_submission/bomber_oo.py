...
class Building():
    ...
    def is_inside(self, point):
        collision_leeway = 6
        if (self.x - collision_leeway <= point.X <= self.x + self.width + collision_leeway) and point.Y >= CANVAS_HEIGHT - self.height:
            return True
        else:
            return False
    ...
    def shrink(self):
        self.height = self.height - BUILDING_SHRINK
...
class Display(Frame):
    ...
    def check_bomb(self):
        ...
        if self.bomb.position.getY() >= CANVAS_HEIGHT + 10:
            self.bomb.explode()
    ...
    def check_plane(self):
        ...
        for building in self.buildings:
            if (building.is_inside(plane_nose)
                ...
                self.game_over()
        if plane_body_bottom.getY() >= CANVAS_HEIGHT - BUILDING_SHRINK and plane_body_bottom.getX() < 20:
            self.plane_landed()
